/**
 * Functionality for d8 logged user.
 */
Drupal.wfm8User = {
  localStoragePrefix: 'wfm8_user_info_',

  isInfoLoaded: function(type) {
    // Check if user info in cookies are the same as in local storage.
    // We can't get UUID ar some other info here because information is JSONencoded and crypted.
    // So we will just store crypted cookie string to local storage and will compare them later to verify if it is the
    // same user.
    if (jQuery.cookie('wfm_public_user') !== localStorage.getItem(this.localStoragePrefix + 'loaded')) {
      // If NOT the same, then we clear all user info local storage records to populate data again.
      for (var i = 0; i < localStorage.length; i++) {
        if (localStorage.key(i).indexOf(this.localStoragePrefix) === 0) {
          localStorage.removeItem(localStorage.key(i));
        }
      }
      localStorage.setItem(this.localStoragePrefix + 'loaded', jQuery.cookie('wfm_public_user'));
    }

    // Check and return if requested info exists in local storage.
    return localStorage.getItem(this.localStoragePrefix + type) !== null;
  },

  setItem: function(type, value) {
    localStorage.setItem(this.localStoragePrefix + type, JSON.stringify(value));
  },
  getItem: function(type) {
    return JSON.parse(localStorage.getItem(this.localStoragePrefix + type));
  },
  unsetItem: function(type) {
    localStorage.removeItem(this.localStoragePrefix + type);
  },

  /**
   * Do a request for User's Lists (recipes, shopping lists, etc.)
   *
   * @param url
   * @param itemName
   * @param add
   * @param callback
   */
  requestLists: function(url, itemName, add, callback) {
    ...
    jQuery.ajax({
      ...
      .done(function (data) {
        Drupal.wfm8User.setItem(itemName, data);
      })
      .always(function (data) {
        if (typeof callback == 'function') {
          callback(Drupal.wfm8User.getItem(itemName));
        }
      });
  },

  requestRecipes: function(add, callback) {
    this.requestLists(Drupal.settings.basePath + 'wfm8-authenticate/user-recipes', 'recipes', add, callback);
  },

  requestShoppingLists: function(callback) {
    this.requestLists(Drupal.settings.basePath + 'wfm8-authenticate/user-shopping-lists', 'shoppingLists', [], callback);
  },

  isLoggedIn: function() {
    return jQuery.cookie('wfm_public_user') !== null;
  },

  /**
   * Get User's recipe box list. Using callbacks because info can be unavailable yet.
   *
   * @param callback
   * @returns {boolean}
   */
  getRecipes: function(callback) {
    ...
    if (this.isInfoLoaded('recipes')) {
      callback(this.getItem('recipes'));
    }
    else {
      this.requestRecipes([], callback);
    }
  },

  removeRecipe: function(id) {
    requestRecipes('remove', id);
  },
  addRecipe: function(id) {
    requestRecipes('add', id);
  },
  ...
  getUserStore: function() {
    return secondaryHeaderMenu.$store.getters.storeModalData.selected_store.id;
  },
   ...
   * @param id
   */
  setUserStore: function(id) {
    var evt = new CustomEvent('onStoreChanged', { detail: {store_id: id} });
  ...
  getShoppingLists: function(callback) {
    ...
    if (this.isInfoLoaded('shoppingLists')) {
      callback(this.getItem('shoppingLists'));
    }
    else {
      this.requestShoppingLists(callback);
    }
  },
   ...
   * @returns {boolean}
   */
  clearShoppingLists: function() {
    this.unsetItem('shoppingLists');
  }
};;/**/
    ...
    filter = {
      ...
      setFilterValue: function(_class, value) {
        ...
        if (_class == 'ugc') {
          //this.setUGC(value);
          if (value == 1) {
            window.location = "/values-matter/your-stories";
          } else {
            window.location = "/values-matter";
          }
          return;
        }
...
Drupal.theme.prototype.brandCampListRow = function(type, elem) {
  ...
  return '<div class="views-row" nid="' + elem.nid + '"><article id="node-' + elem.nid + '" class="node node-bc-' + (elem.type ? elem.type : type) + ' node-teaser article clearfix">'
      + '<header class="node-header"><h1 property="dc:title" datatype="" class="node-title"><a href="' + elem.url
    + '" rel="bookmark" class="no-colorbox-node" data-inner-width="1060" data-inner-height="754">'
      ...
      + '<a href="' + elem.url
    //+ '" data-product-id="' + elem.nid + '" ><img width="'
    + '" data-product-id="' + elem.nid
    + '" data-product-category="' + elem.category
    + '" data-product-hasVideo="' + elem.hasvideo
    + '" data-product-list="' + elem.currentpath
    + '" data-product-type="Tile'
    + '" data-product-position="' + elem.weight + '" ><img width="'
    + dimensions.bc.width + '" '+ image +'></a></figure></div></div>'
      + content
